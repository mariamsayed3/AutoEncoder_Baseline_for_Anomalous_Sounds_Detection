...
def file_to_vectors(file_name,
    ...
    # Improvement1
    if file_name[(file_name.find("section")) + 9] == '0' and file_name.find("train") > -1:
        print("hello")
        log_mel_spectrogram = spec_augmentMod(log_mel_spectrogram)
...
def freq_mask(mel_spectrogram, frequency_masking_para=27, frequency_mask_num=2):
    v = mel_spectrogram.shape[1]  # no. of mel bins
    # apply m_F frequency masks to the mel spectrogram
    for i in range(frequency_mask_num):
        f = int(np.random.uniform(0, frequency_masking_para))  # [0, F)
...
def time_mask(mel_spectrogram, time_masking_para=100, time_mask_num=2):
    tau = mel_spectrogram.shape[1]  # time frames
    # apply m_T time masks to the mel spectrogram
    for i in range(time_mask_num):
        t = int(np.random.uniform(0, time_masking_para))  # [0, T)
        t0 = random.randint(0, tau - t)  # [0, tau - t)
        mel_spectrogram[:, t0:t0 + t] = 0
    return mel_spectrogram


def spec_augmentMod(mel_spectrogram):
    frequency_spectrogram = freq_mask(mel_spectrogram)
    frequency_time_sepctrogram = time_mask(frequency_spectrogram)
